...
import { registerUserManagementRoutes } from "../modules/userManagementRouteModule.js";
import { getUserRolesForUser, resolveAssignedRoles, sameRoleList } from "../../services/userRoleService.js";
...
function registerUserServiceRoutes(app: RouteRegistrar, deps: UserRouteDeps) {
  ...
  async function fetchUserById(userId: number) {
    ...
    const effectiveRoles = resolveAssignedRoles({
      role: user.role,
      roles: user.roles,
      fallbackRole: user.role,
    });
    user.assigned_branch_ids = effectiveRoles.some((role: string) => supportsBranchAssignments(role))
  ...
  async function resolveRoleAssignments({
    ...
    let selectedBranch: Record<string, any> | null | undefined = null;
    if (branchId) {
    ...
    if (normalizedRole === "operations_manager" || normalizedRole === "loan_officer" || normalizedRole === "cashier") {
      ...
      let resolvedBranch: Record<string, any> | null | undefined = selectedBranch;
      let resolvedBranchId = branchId;
      if (!resolvedBranchId && branchIds.length === 1) {
        resolvedBranchId = branchIds[0] ?? null;
    ...
    if (normalizedRole === "area_manager") {
      ...
      if (branchCount) {
        ...
        if (resolvedBranchIds.length === 0) {
          throw createHttpError(400, "Explicit branchIds are required when branchCount is provided for area_manager role");
        }
      }
      if (resolvedBranchIds.length === 0) {
        const candidateBranches = await hierarchyService.getBranches({
        ...
        resolvedBranchIds = [Number(candidateBranches[0]!.id)];
      }
      ...
      return { branchId: null, primaryRegionId: regionIds[0] ?? null, areaBranchIds: resolvedBranchIds };
    }
    ...
    if (normalizedRole === "investor" || normalizedRole === "partner") {
      ...
      const resolvedBranchIds = branchIds.length > 0 ? branchIds : (branchId ? [branchId] : []);
      if (branchCount) {
        if (resolvedBranchIds.length > 0 && resolvedBranchIds.length !== branchCount) {
          throw createHttpError(400, "branchCount must match the number of provided branchIds");
        }
        if (resolvedBranchIds.length === 0) {
          throw createHttpError(400, `Explicit branchIds are required when branchCount is provided for ${normalizedRole} role`);
        }
      }
      if (resolvedBranchIds.length === 0) {
        throw createHttpError(
          400,
          `${normalizedRole === "investor" ? "Investors" : "Partners"} must be assigned using branchIds or branchId`,
        );
      }
      const branches = await hierarchyService.getBranchesByIds(resolvedBranchIds, { requireActive: true });
      if (branches.length !== resolvedBranchIds.length) throw createHttpError(400, "One or more assigned branches are invalid or inactive");
      if (primaryRegionId) {
        ...
        const regionIds = [...new Set(branches.map((branch: Record<string, any>) => Number(branch.region_id)).filter(Boolean))];
        if (regionIds.length === 1) primaryRegionId = regionIds[0] ?? null;
      }
  ...
  async function getAdminContinuityViolation({
    ...
    const currentRoles = resolveAssignedRoles({
      ...
      fallbackRole: targetUser.role,
    });
    const resolvedNextRoles = resolveAssignedRoles({
      role: nextRole,
      roles: nextRoles,
      fallbackRole: nextRole || targetUser.role,
    });
    const targetIsActiveAdmin = currentRoles.includes("admin") && Number(targetUser.is_active) === 1;
    const remainsActiveAdmin = resolvedNextRoles.includes("admin") && nextIsActive === 1;
    if (targetUser.id === actingUserId && nextIsActive !== 1) return { status: 400, message: "You cannot deactivate your own account" };
    if (targetUser.id === actingUserId && currentRoles.includes("admin") && !resolvedNextRoles.includes("admin")) {
