...
type LoanCollateralRouteOptions = {
  ...
  resolveRiskRecordBranchId: (user: RiskRouteUser, requestedBranchId: number | null | undefined) => Promise<number>;
  resolveLoanInScope: (loanId: number, user: RiskRouteUser) => Promise<{ scope: any; loan: Record<string, any> }>;
  ...
  loanUnderwritingService: {
    refreshLoanAssessment: (loanId: number) => Promise<any>;
  };
};
...
function registerLoanCollateralRoutes(options: LoanCollateralRouteOptions) {
  ...
  function createTenantIsolationError(tableName: string): Error & { status: number } {
    const error = new Error(
      `Tenant isolation is unavailable because ${tableName}.tenant_id is missing. Run the required schema repair before serving this endpoint.`,
    ) as Error & { status: number };
    error.status = 503;
    return error;
  }

  // Cache PRAGMA results so the check runs at most once per table per process lifetime.
  // A missing-column result is never cached — a schema repair + restart clears naturally.
  const _tenantColumnVerified = new Map<string, true>();

  async function assertTableHasTenantColumn(tableName: string): Promise<void> {
    if (_tenantColumnVerified.has(tableName)) {
      return;
    }
    try {
      const columns = await all(`PRAGMA table_info(${tableName})`);
      const hasTenantId = columns.some((column) => String(column.name || "").toLowerCase() === "tenant_id");
      if (!hasTenantId) {
        throw createTenantIsolationError(tableName);
      }
      _tenantColumnVerified.set(tableName, true);
    } catch (err) {
  ...
  // -- GET /api/collateral-assets/:id ------------------------------------------
  app.get("/api/collateral-assets/:id", authenticate, authorize(...collateralViewRoles), async (req, res, next) => {
    try {
      const tenantId = getCurrentTenantId();
      const assetId = parseId(req.params.id);
      if (!assetId) { res.status(400).json({ message: "Invalid collateral asset id" }); return; }
      ...
      res.status(200).json(collateralAsset);
    } catch (error) {
      next(error);
    }
  });

  // -- PATCH /api/collateral-assets/:id ----------------------------------------
  app.patch("/api/collateral-assets/:id", authenticate, authorize(...collateralManageRoles), async (req, res, next) => {
    try {
      const tenantId = getCurrentTenantId();
      const assetId = parseId(req.params.id);
      if (!assetId) { res.status(400).json({ message: "Invalid collateral asset id" }); return; }

      const payload = updateCollateralAssetSchema.parse(req.body || {});
      const scope = await hierarchyService.resolveHierarchyScope(req.user);
      ...
      if (!existing) { res.status(404).json({ message: "Collateral asset not found" }); return; }
      if (!hierarchyService.isBranchInScope(scope, existing.branch_id)) {
        res.status(403).json({ message: "Forbidden: collateral asset is outside your scope" });
        return;
      }
      ...
      if (payload.registrationNumber != null) {
        const dup = await get(
          `SELECT id FROM collateral_assets
           WHERE tenant_id = ? AND LOWER(TRIM(COALESCE(registration_number, ''))) = LOWER(TRIM(?)) AND id != ?`,
          [tenantId, payload.registrationNumber, assetId],
        );
        if (dup) {
          res.status(409).json({ message: "A collateral asset with this registration number already exists" });
          return;
      ...
      if (payload.logbookNumber != null) {
        const dup = await get(
          `SELECT id FROM collateral_assets
           WHERE tenant_id = ? AND LOWER(TRIM(COALESCE(logbook_number, ''))) = LOWER(TRIM(?)) AND id != ?`,
          [tenantId, payload.logbookNumber, assetId],
        );
        if (dup) {
          res.status(409).json({ message: "A collateral asset with this logbook number already exists" });
          return;
      ...
      if (payload.titleNumber != null) {
        const dup = await get(
          `SELECT id FROM collateral_assets
           WHERE tenant_id = ? AND LOWER(TRIM(COALESCE(title_number, ''))) = LOWER(TRIM(?)) AND id != ?`,
          [tenantId, payload.titleNumber, assetId],
        );
        if (dup) {
          res.status(409).json({ message: "A collateral asset with this title number already exists" });
          return;
      ...
      if (payload.assetType !== undefined)          { setClauses.push("asset_type = ?");           setParams.push(payload.assetType); }
      if (payload.description !== undefined)        { setClauses.push("description = ?");          setParams.push(payload.description); }
      if (payload.estimatedValue !== undefined)     { setClauses.push("estimated_value = ?");      setParams.push(payload.estimatedValue); }
      if (payload.ownershipType !== undefined)      { setClauses.push("ownership_type = ?");       setParams.push(payload.ownershipType); }
      if (payload.ownerName !== undefined)          { setClauses.push("owner_name = ?");           setParams.push(payload.ownerName); }
      if (payload.ownerNationalId !== undefined)    { setClauses.push("owner_national_id = ?");    setParams.push(payload.ownerNationalId); }
      if (payload.registrationNumber !== undefined) { setClauses.push("registration_number = ?");  setParams.push(payload.registrationNumber); }
      if (payload.logbookNumber !== undefined)      { setClauses.push("logbook_number = ?");       setParams.push(payload.logbookNumber); }
      if (payload.titleNumber !== undefined)        { setClauses.push("title_number = ?");         setParams.push(payload.titleNumber); }
      if (payload.locationDetails !== undefined)    { setClauses.push("location_details = ?");     setParams.push(payload.locationDetails); }
      if (payload.valuationDate !== undefined)      { setClauses.push("valuation_date = ?");       setParams.push(payload.valuationDate); }
      if (payload.status !== undefined)             { setClauses.push("status = ?");               setParams.push(payload.status); }

      await run(
        `UPDATE collateral_assets SET ${setClauses.join(", ")} WHERE id = ? AND tenant_id = ?`,
        [...setParams, assetId, tenantId],
      );
      ...
      await writeAuditLog({
        userId: req.user.sub,
        action: "collateral_asset.updated",
        targetType: "collateral_asset",
        targetId: assetId,
        details: JSON.stringify({ updatedFields: Object.keys(payload) }),
        ipAddress: req.ip,
      });
  ...
  app.post("/api/loans/:id/guarantors", authenticate, authorize(...collateralManageRoles), async (req, res, next) => {
    try {
      ...
      loanUnderwritingService.refreshLoanAssessment(loanId).catch(() => {});
  ...
  app.get("/api/loans/:id/guarantors", authenticate, authorize(...collateralViewRoles), async (req, res, next) => {
    try {
      ...
      await assertTableHasTenantColumn("loan_guarantors");
  ...
  app.delete("/api/loans/:id/guarantors/:loanGuarantorId", authenticate, authorize(...collateralManageRoles), async (req, res, next) => {
    try {
      ...
      const { scope } = await resolveLoanInScope(loanId, req.user);
      const linkedGuarantor = await get(
        `SELECT lg.id, lg.loan_id, lg.guarantor_id, lg.guarantee_amount, lg.liability_type,
                g.full_name, g.branch_id
         FROM loan_guarantors lg
         INNER JOIN guarantors g ON g.id = lg.guarantor_id
      ...
      if (!hierarchyService.isBranchInScope(scope, linkedGuarantor.branch_id)) {
        res.status(403).json({ message: "Forbidden: guarantor is outside your scope" });
        return;
      }
  ...
  app.get("/api/loans/:id/collaterals", authenticate, authorize(...collateralViewRoles), async (req, res, next) => {
    try {
      ...
      await resolveLoanInScope(loanId, req.user);
      await assertTableHasTenantColumn("loan_collaterals");
  ...
  app.delete("/api/loans/:id/collaterals/:loanCollateralId", authenticate, authorize(...collateralManageRoles), async (req, res, next) => {
    try {
      ...
      const { scope } = await resolveLoanInScope(loanId, req.user);
      const linkedCollateral = await get(
        `SELECT lc.id, lc.loan_id, lc.collateral_asset_id, lc.forced_sale_value, lc.lien_rank,
                ca.description, ca.status, ca.branch_id
         FROM loan_collaterals lc
         INNER JOIN collateral_assets ca ON ca.id = lc.collateral_asset_id
      ...
      if (!hierarchyService.isBranchInScope(scope, linkedCollateral.branch_id)) {
        res.status(403).json({ message: "Forbidden: collateral asset is outside your scope" });
        return;
      }
