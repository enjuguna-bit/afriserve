...
function registerLoanStatementRoutes(options: LoanStatementRouteOptions) {
  ...
  function mapRepaymentPhone<T extends Record<string, any>>(repayment: T): T {
    return {
      ...repayment,
      payer_phone: formatKenyanPhoneDisplay(repayment?.payer_phone),
    };
  }
    ...
    try {
      ...
      if (isCollectibleLoanStatus(loan.status)) {
        try {
          ...
          // Statement reads should degrade gracefully when overdue refresh is temporarily unavailable.
        }
      }
      ...
      let breakdown: Record<string, any> | null | undefined = null;
      try {
        breakdown = await getLoanBreakdown(loanId);
      } catch {
        breakdown = null;
      }
      ...
      try {
        workflow = await getLoanWorkflowSnapshot({ get, loanId });
      } catch {
        workflow = null;
      }

      let underwriting: Record<string, unknown> | null | undefined = null;
      if (loanUnderwritingService) {
        try {
          underwriting = await loanUnderwritingService.refreshLoanAssessment(loanId);
        } catch {
          underwriting = null;
        }
      }
      ...
      res.status(200).json({
        ...
        amortization,
        repayments: repayments.map((repayment) => mapRepaymentPhone(repayment)),
      });
    } catch (error) {
      next(error);
    }
  ...
  app.get("/api/loans/:id/repayments", authenticate, async (req, res, next) => {
    try {
      ...
      res.status(200).json(repayments.map((repayment) => mapRepaymentPhone(repayment)));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/loans/:id/installments", authenticate, async (req, res, next) => {
    try {
      ...
      const scope = await hierarchyService.resolveHierarchyScope(req.user);
      const loan = await get("SELECT id, branch_id, status FROM loans WHERE id = ? AND tenant_id = ?", [loanId, getCurrentTenantId()]);
      if (!loan) {
        res.status(404).json({ message: "Loan not found" });
        return;
      }
