...
import { resolveAssignedRoles, selectMostRestrictiveRole } from "../../services/userRoleService.js";

const hqVisibilityRoles = new Set(["admin", "ceo", "finance", "it"]);
const assignmentVisibilityRoles = new Set(["area_manager", "investor", "partner"]);

function resolveUserVisibilityRole(user: Record<string, any> | null | undefined): string {
  const resolvedRoles = resolveAssignedRoles({
    role: user?.role,
    roles: user?.roles,
    fallbackRole: user?.role,
  });
  return selectMostRestrictiveRole(resolvedRoles) || String(user?.role || "").trim().toLowerCase();
}
...
function isUserVisibleInScope(
  ...
  const visibilityRole = resolveUserVisibilityRole(user);
  if (hqVisibilityRoles.has(visibilityRole)) {
    return false;
  }

  const assignedBranchIds = normalizeIds(Array.isArray(user.assigned_branch_ids) ? user.assigned_branch_ids : []);
  if (assignmentVisibilityRoles.has(visibilityRole)) {
...
function registerUserReadRoutes(options: UserManagementRouteOptions) {
  ...
  app.get("/api/users", authenticate, readUsersPermission, async (req, res, next) => {
    try {
  ...
  app.get("/api/users/roles", authenticate, readUsersPermission, async (req, res, next) => {
    try {
      const scope = await hierarchyService.resolveHierarchyScope(req.user);
  ...
  app.get("/api/users/summary", authenticate, readUsersPermission, async (req, res, next) => {
    try {
      const scope = await hierarchyService.resolveHierarchyScope(req.user);
  ...
  app.get("/api/users/:id", authenticate, readUsersPermission, async (req, res, next) => {
    try {
  ...
  app.get("/api/users/:id/security-state", authenticate, readUsersPermission, async (req, res, next) => {
    try {
      const userId = parseUserIdOrRespond(req, res);
      if (!userId) return;
