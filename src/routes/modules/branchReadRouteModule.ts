...
function registerBranchReadRoutes(options: BranchManagementRouteOptions) {
      ...
      const loanStats = await get(
        `
          SELECT
            ...
            SUM(CASE WHEN status IN ('active', 'restructured', 'overdue') THEN 1 ELSE 0 END) AS active_loans,
            ...
            COALESCE(SUM(CASE WHEN disbursed_at IS NOT NULL THEN principal ELSE 0 END), 0) AS principal_disbursed,
            COALESCE(SUM(expected_total), 0) AS expected_total,
            COALESCE(SUM(repaid_total), 0) AS repaid_total,
            COALESCE(SUM(CASE WHEN status IN ('active', 'restructured', 'overdue') THEN balance ELSE 0 END), 0) AS outstanding_balance,
  ...
  app.get("/api/hierarchy/events", authenticate, async (req, res, next) => {
    try {
      ...
      res.status(200).json({ data: events, nextSinceId: lastEvent?.id ?? sinceId });
    } catch (error) {
      next(error);
    }
  });
