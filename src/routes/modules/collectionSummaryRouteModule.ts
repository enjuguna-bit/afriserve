    ...
    async (req, res, next) => {
      try {
        ...
        async function computeSummaryPayload() {
          ...
          const scopedLoanParams = [tenantId, ...scopeCondition.params, ...(filteredOfficerId != null ? [filteredOfficerId] : [])];
          ...
          const actionScopeSql = scopeCondition.sql
            ...
            : "";
          // officer filter reuses same params order: tenantId, ...scopeParams, ...officerParams
            ...
            ? await get(
              `
                ...
                WHERE i.status != 'paid'
                  ...
                  AND l.officer_id = ?
              `,
              [tenantId, ...scopeCondition.params, quickCountOfficerId],
            )
            : null;
