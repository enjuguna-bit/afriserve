    ...
    try {
      ...
      const summary = await get(
        `
          SELECT
            ...
            SUM(CASE WHEN l.status IN ('active', 'restructured', 'overdue') THEN 1 ELSE 0 END) AS active_loans,
            ROUND(COALESCE(SUM(l.repaid_total), 0), 2) AS repaid_total,
            ROUND(COALESCE(SUM(CASE WHEN l.status IN ('active', 'restructured', 'overdue') THEN l.balance ELSE 0 END), 0), 2) AS outstanding_balance
          FROM loans l
          LEFT JOIN clients c ON c.id = l.client_id
          ${scopeSql}
          ${scopeSql ? "AND" : "WHERE"} l.tenant_id = ?
        `,
        [...scopeCondition.params, tenantId],
      );
      ...
      const branchPerformance = await all(
        `
          ...
          LEFT JOIN loans l ON l.branch_id = b.id AND l.tenant_id = ?
          ${scopeCondition.sql ? `WHERE ${scopeCondition.sql.replace(/l\.branch_id/g, "b.id")}` : "WHERE 1=1"}
          GROUP BY b.id, b.name, b.code, r.id, r.name
          ORDER BY r.name ASC, b.name ASC
        `,
        [tenantId, ...scopeCondition.params],
      );
      ...
      const roPerformance = await all(
        `
          ...
          LEFT JOIN loans l ON l.created_by_user_id = u.id AND l.tenant_id = ?
          WHERE u.role = 'loan_officer'
            AND u.tenant_id = ?
            ${scopeCondition.sql ? `AND ${scopeCondition.sql.replace(/l\.branch_id/g, "u.branch_id")}` : ""}
          GROUP BY u.id, u.full_name, u.email, u.branch_id, b.name
          ORDER BY b.name ASC, u.full_name ASC
        `,
        [tenantId, tenantId, ...scopeCondition.params],
      );
