...
import { replaceUserRoles, resolveAssignedRoles, resolvePrimaryRole } from "../../services/userRoleService.js";
...
function registerUserAccountActionRoutes(options: UserManagementRouteOptions) {
  ...
  app.post("/api/users", authenticate, ...applyRbacPolicy("users.create", authorize), async (req, res, next) => {
    try {
      ...
      const existingUser = await get("SELECT id FROM users WHERE LOWER(email) = ? AND tenant_id = ?", [normalizedEmail, getCurrentTenantId()]);
      ...
      const insert = await run(
        `
          INSERT INTO users (tenant_id, full_name, email, password_hash, role, branch_id, primary_region_id, created_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
        `,
        [getCurrentTenantId(), payload.fullName, normalizedEmail, passwordHash, primaryRole, assignments.branchId, assignments.primaryRegionId],
      );
    ...
    try {
      ...
      if (typeof payload.email === "string") {
        ...
        if (normalized !== user.email) {
          const existingEmail = await get("SELECT id FROM users WHERE LOWER(email) = ? AND id != ? AND tenant_id = ?", [normalized, userId, getCurrentTenantId()]);
      ...
      if (setClauses.length > 0) {
        await run(
          ...
          `,
          [...queryParams, userId, getCurrentTenantId()],
        );
    ...
    try {
      ...
      await run(
        `
          ...
          WHERE id = ? AND tenant_id = ?
        `,
        [nextPrimaryRole, assignment.branchId, assignment.primaryRegionId, userId, getCurrentTenantId()],
      );
  ...
  app.post("/api/users/:id/revoke-sessions", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const userId = parseUserIdOrRespond(req, res);
      if (!userId) return;
      const user = await fetchUserById(userId);
      if (!user) { res.status(404).json({ message: "User not found" }); return; }
      await run("UPDATE users SET token_version = token_version + 1 WHERE id = ? AND tenant_id = ?", [userId, getCurrentTenantId()]);
      await invalidateCachedAuthSessionUser(userId);
      hierarchyService.invalidateHierarchyCaches({ userId });
      await writeAuditLog({
        userId: req.user.sub,
  ...
  app.post("/api/users/:id/unlock", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const userId = parseUserIdOrRespond(req, res);
      if (!userId) return;
      const user = await fetchUserById(userId);
      if (!user) { res.status(404).json({ message: "User not found" }); return; }
      await run("UPDATE users SET failed_login_attempts = 0, locked_until = NULL WHERE id = ? AND tenant_id = ?", [userId, getCurrentTenantId()]);
      await writeAuditLog({
        userId: req.user.sub,
  ...
  app.post("/api/users/:id/reset-password", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const userId = parseUserIdOrRespond(req, res);
      if (!userId) return;
      ...
      const passwordHash = await bcrypt.hash(payload.newPassword, bcryptRounds);

      await run(
        `
          UPDATE users
          SET password_hash = ?, failed_login_attempts = 0, locked_until = NULL, token_version = token_version + 1
          WHERE id = ? AND tenant_id = ?
        `,
        [passwordHash, userId, getCurrentTenantId()],
      );
      ...
      await publishUserDomainEvent("user.password.admin_reset", userId, req.user.sub, {
        email: user.email,
      });

      const updated = await fetchUserById(userId);
      res.status(200).json({ message: "User password has been reset", user: sanitizeUserRow(updated) });
    } catch (error) { next(error); }
  });

  app.post("/api/users/:id/reset-token", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const userId = parseUserIdOrRespond(req, res);
      if (!userId) return;
      const user = await fetchUserById(userId);
    ...
    try {
      ...
      const customPermissions = await all(
        `
          ...
          INNER JOIN users target_user ON target_user.id = ucp.user_id AND target_user.tenant_id = ?
          INNER JOIN permissions p ON p.permission_id = ucp.permission_id
          LEFT JOIN users grantor ON grantor.id = ucp.granted_by_user_id AND grantor.tenant_id = target_user.tenant_id
          WHERE ucp.user_id = ?
    ...
    try {
      ...
      const alreadyGranted = await get(
        `
          ...
          WHERE ucp.user_id = ? AND ucp.permission_id = ? AND u.tenant_id = ?
          LIMIT 1
        `,
        [userId, permissionId, getCurrentTenantId()],
      );
      ...
      await run(
        ...
        `,
        [userId, permissionId, req.user.sub, userId, getCurrentTenantId()],
      );
    ...
    async (req, res, next) => {
      try {
        ...
        const deleteResult = await run(
          ...
          `,
          [userId, permissionId, userId, getCurrentTenantId()],
        );
  ...
  app.post("/api/users/:id/deactivate", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const userId = parseUserIdOrRespond(req, res);
      if (!userId) return;
      const user = await fetchUserById(userId);
      ...
      const updateResult = await run(
        "UPDATE users SET is_active = 0, deactivated_at = datetime('now'), failed_login_attempts = 0, locked_until = NULL, token_version = token_version + 1 WHERE id = ? AND tenant_id = ? AND is_active = 1",
  ...
  app.post("/api/users/:id/activate", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const userId = parseUserIdOrRespond(req, res);
      if (!userId) return;
      const user = await fetchUserById(userId);
      ...
      await run("UPDATE users SET is_active = 1, deactivated_at = NULL WHERE id = ? AND tenant_id = ?", [userId, getCurrentTenantId()]);
      await invalidateCachedAuthSessionUser(userId);
      hierarchyService.invalidateHierarchyCaches({ userId });
      await writeAuditLog({
        userId: req.user.sub,
  ...
  app.delete("/api/users/:id", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const userId = parseUserIdOrRespond(req, res);
      if (!userId) return;
      const user = await fetchUserById(userId);
      ...
      await run("DELETE FROM password_resets WHERE user_id = ? AND tenant_id = ?", [userId, getCurrentTenantId()]);
      await run(
        "UPDATE users SET is_active = 0, deactivated_at = datetime('now'), failed_login_attempts = 0, locked_until = NULL, token_version = token_version + 1 WHERE id = ? AND tenant_id = ?",
        [userId, getCurrentTenantId()],
      );
      await invalidateCachedAuthSessionUser(userId);
