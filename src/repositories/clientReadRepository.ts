...
function createClientReadRepository(deps: ClientReadRepositoryDeps) {
  ...
  async function listClients(filters: ListClientsFilters) {
    ...
    // Always scope reads to the current tenant — works for both SQLite (no RLS)
    // and Postgres (belt-and-suspenders alongside the RLS policy).
    where.addEquals("c.tenant_id", getCurrentTenantId());
  ...
  async function findPotentialDuplicateCandidates(filters: FindDuplicateCandidatesFilters) {
    ...
    // Always scope duplicate candidates to the current tenant.
    where.addEquals("c.tenant_id", getCurrentTenantId());
