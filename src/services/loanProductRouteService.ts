...
function registerLoanProductRoutes(options: LoanProductRouteOptions) {
  ...
  app.post("/api/loan-products", authenticate, authorize("admin"), async (req, res, next) => {
    try {
      ...
      const duplicate = await get(
        "SELECT id FROM loan_products WHERE LOWER(TRIM(name)) = LOWER(TRIM(?))",
        [payload.name],
      );
      if (duplicate) {
      ...
      const createdProduct = await get("SELECT * FROM loan_products WHERE id = ?", [insertResult.lastID]);
      await writeAuditLog({
        userId: req.user.sub,
  ...
  app.patch("/api/loan-products/:id", authenticate, authorize("admin"), async (req, res, next) => {
    try {
      ...
      const existingProduct = await get("SELECT * FROM loan_products WHERE id = ?", [productId]);
      if (!existingProduct) {
      ...
      if (payload.name) {
        const duplicate = await get(
          "SELECT id FROM loan_products WHERE LOWER(TRIM(name)) = LOWER(TRIM(?)) AND id != ?",
      ...
      const updatedProduct = await get("SELECT * FROM loan_products WHERE id = ?", [productId]);
      await writeAuditLog({
        userId: req.user.sub,
