...
function registerUserReadRoutes(options: UserManagementRouteOptions) {
  ...
  app.get("/api/users", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
  ...
  app.get("/api/users/roles", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const scope = await hierarchyService.resolveHierarchyScope(req.user);
  ...
  app.get("/api/users/summary", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
      const scope = await hierarchyService.resolveHierarchyScope(req.user);
  ...
  app.get("/api/users/:id", authenticate, manageUsersPermission, async (req, res, next) => {
    try {
