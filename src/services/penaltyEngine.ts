...
function createPenaltyEngine(options: PenaltyEngineOptions) {
  ...
  // FIX #2: Changed INNER JOIN loan_products to LEFT JOIN so that overdue installments
  // on loans without a product_id are still included in penalty processing,
  // provided the installment-level penalty config is set directly on the installment row.
  ...
  async function applyPenaltyToInstallment(
    ...
    const chargeApplied = await executeTransaction(async (tx) => {
      // FIX #2 (cont.): Also use LEFT JOIN here for the same reason.
