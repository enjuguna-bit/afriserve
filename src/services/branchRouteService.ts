...
import { registerBranchManagementRoutes } from "./branchManagementRouteModule.js";
...
function registerBranchServiceRoutes(app: RouteRegistrar, deps: BranchRouteDeps) {
  ...
  async function getBranchDeletionDependencies(branchId: number) {
    const row = await get(
      `
        SELECT
          (SELECT COUNT(*) FROM users WHERE branch_id = ?) AS users_assigned,
          (SELECT COUNT(*) FROM clients WHERE branch_id = ?) AS clients_assigned,
          (SELECT COUNT(*) FROM loans WHERE branch_id = ?) AS loans_linked,
          (SELECT COUNT(*) FROM transactions WHERE branch_id = ?) AS transactions_linked,
          (SELECT COUNT(*) FROM collection_actions WHERE branch_id = ?) AS collection_actions_linked
      `,
