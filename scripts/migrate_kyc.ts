...
function ensureColumn(tableName: string, definitionSql: string): void {
...
function ensureLoanStatusConstraint() {
  ...
  const hasLoanColumn = (columnName: string): boolean => loanColumns.some((column) => column.name === columnName);
  const selectColumnOrNull = (columnName: string): string => (hasLoanColumn(columnName) ? columnName : `NULL AS ${columnName}`);
