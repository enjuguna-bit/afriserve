...
if (-not $DB_PASS) {
    throw "AFRISERVE_DB_PASSWORD must be set before running deploy-webapp.ps1."
}
...
$DB_HOST = ([string](az postgres flexible-server show --resource-group $APP_RG --name $DB_SERVER --query fullyQualifiedDomainName -o tsv)).Trim()
$REDIS_KEY = ([string](az redis list-keys --name $REDIS_NAME --resource-group $APP_RG --query primaryKey -o tsv)).Trim()
...
az webapp config appsettings set --name $APP_NAME --resource-group $APP_RG --settings $settings -o none
