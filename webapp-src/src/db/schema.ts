...
async function executePrismaSchemaSync(): Promise<string> {
  const effectiveEnv: NodeJS.ProcessEnv = {
    ...process.env,
  };

  if (!String(effectiveEnv.DATABASE_URL || "").trim() && dbClient === "sqlite") {
    const sqlitePath = String(dbPath || "").trim();
    effectiveEnv.DATABASE_URL = sqlitePath === ":memory:"
      ? "file:./data/microfinance.db"
  ...
  try {
    const { stdout, stderr } = await execFileAsync(
      getNpxCommand(),
      ["prisma", "db", "push", "--skip-generate", "--schema", resolvePrismaSchemaPath()],
      {
        cwd: repoRoot,
        env: effectiveEnv,
        windowsHide: true,
    ...
    const { stdout, stderr } = await execFileAsync(
      process.execPath,
      [prismaCliPath, "db", "push", "--skip-generate", "--schema", resolvePrismaSchemaPath()],
      {
        cwd: repoRoot,
        env: effectiveEnv,
        windowsHide: true,
...
async function runMigrations(options: RunMigrationsOptions = {}): Promise<RunMigrationsResult> {
  ...
  try {
    prismaOutput = await executePrismaSchemaSync();
    ...
    skipped = ["prisma_schema_sync_skipped_database_locked"];
    await bootstrapSqliteSchemaFromPrismaMigrations();
    await ensureSqliteRuntimeCompatibilitySchema();
  }

  const applied = dbClient === "postgres"
    ? ["prisma_db_push_postgres_schema_sync"]
    : [];
