...
function registerSystemRoutes(app: RouteRegistrarApp, deps: SystemRouteDeps): void {
  ...
  app.get("/metrics", authenticate, authorize("admin", "it"), systemConfigPermission, (_req: Request, res: Response) => {
  ...
  app.get("/api/system/service-catalog", authenticate, authorize("admin", "it"), systemConfigPermission, async (_req: Request, res: Response) => {
  ...
  app.get("/api/system/status", authenticate, authorize("admin"), systemConfigPermission, async (_req: Request, res: Response) => {
    const runtimeStatus = await getRuntimeStatus();
    res.status(200).json(runtimeStatus);
  });

  app.get("/api/system/config", authenticate, authorize("admin"), systemConfigPermission, (_req: Request, res: Response) => {
  ...
  app.get("/api/system/config-status", authenticate, authorize("admin"), systemConfigPermission, (_req: Request, res: Response) => {
    res.status(200).json(getConfigStatus());
  });

  app.get("/api/system/metrics", authenticate, authorize("admin"), systemConfigPermission, (_req: Request, res: Response) => {
    if (!metrics || typeof metrics.getSnapshot !== "function") {
