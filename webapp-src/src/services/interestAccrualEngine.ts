...
function createInterestAccrualEngine(options: InterestAccrualEngineOptions) {
  ...
  async function getCandidates(limit: number, offset: number) {
    return all(
      `
        SELECT
          l.id AS loan_id,
        ...
        WHERE LOWER(TRIM(COALESCE(p.accrual_method, 'upfront'))) = 'daily_eod'
  ...
  async function applyDailyAccruals(): Promise<AccrualSummary> {
    ...
    while (true) {
      const batch = await getCandidates(batchSize, offset);
      if (batch.length === 0) {
        break;
      }
