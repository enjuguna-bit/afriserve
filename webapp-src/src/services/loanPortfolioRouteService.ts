...
function registerLoanPortfolioRoutes(options: LoanPortfolioRouteOptions) {
  ...
  app.post("/api/loans", authenticate, authorize("admin", "loan_officer"), async (req, res, next) => {
    try {
  ...
  app.get("/api/loans/pending-approval", authenticate, authorize("admin", "operations_manager", "area_manager"), async (req, res, next) => {
    try {
      const scope = await hierarchyService.resolveHierarchyScope(req.user);
  ...
  app.patch("/api/loans/:id/assign-officer", authenticate, authorize("admin", "operations_manager", "area_manager"), async (req, res, next) => {
    try {
      const loanId = parseId(req.params.id);
      if (!loanId) {
        res.status(400).json({ message: "Invalid loan id" });
      ...
      if (previousOfficerId === payload.officerId) {
        const unchangedLoan = await get("SELECT * FROM loans WHERE id = ?", [loanId]);
