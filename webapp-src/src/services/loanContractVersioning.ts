...
async function buildLoanContractSnapshotTx(
  ...
  const trancheSummaryRows = await tx.$queryRawUnsafe<Array<{
    total_disbursed: number | null;
    tranche_count: number | null;
    final_tranche_count: number | null;
  }>>(
    `
      SELECT
...
async function recordLoanContractVersionTx(
  ...
  const maxVersionRows = await tx.$queryRawUnsafe<Array<{ max_version: number | null }>>(
