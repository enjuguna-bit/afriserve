...
function registerLoanCollateralRoutes(options: LoanCollateralRouteOptions) {
  ...
  app.post("/api/loans/:id/guarantors", authenticate, authorize(...collateralManageRoles), async (req, res, next) => {
    try {
      ...
      const guarantor = await get("SELECT * FROM guarantors WHERE id = ?", [payload.guarantorId]);
      if (!guarantor) {
      ...
      const existingLink = await get(
        "SELECT id FROM loan_guarantors WHERE loan_id = ? AND guarantor_id = ?",
        [loanId, payload.guarantorId],
      );
      if (existingLink) {
    ...
    try {
      ...
      const linkedGuarantor = await get(
        `
          ...
          FROM loan_guarantors lg
          INNER JOIN guarantors g ON g.id = lg.guarantor_id
          WHERE lg.id = ? AND lg.loan_id = ?
        `,
        [loanGuarantorId, loanId],
      );
      if (!linkedGuarantor) {
      ...
      await run("DELETE FROM loan_guarantors WHERE id = ? AND loan_id = ?", [loanGuarantorId, loanId]);
      ...
      res.status(200).json({
        message: "Guarantor unlinked from loan",
        removedLink: linkedGuarantor,
      });
    } catch (error) {
      next(error);
    }
  ...
  app.post("/api/loans/:id/collaterals", authenticate, authorize(...collateralManageRoles), async (req, res, next) => {
    try {
      ...
      const collateralAsset = await get("SELECT * FROM collateral_assets WHERE id = ?", [payload.collateralAssetId]);
      if (!collateralAsset) {
        res.status(404).json({ message: "Collateral asset not found" });
        return;
      }
      ...
      const existingLink = await get(
        "SELECT id FROM loan_collaterals WHERE loan_id = ? AND collateral_asset_id = ?",
        [loanId, payload.collateralAssetId],
      );
      if (existingLink) {
      ...
      const linkedCollateral = await get(
        `
          SELECT
            ...
            lc.loan_id,
            lc.collateral_asset_id,
            lc.forced_sale_value,
            lc.lien_rank,
            lc.note,
            lc.created_at,
            ca.asset_type,
    ...
    try {
      ...
      const linkedCollateral = await get(
        `
          ...
          FROM loan_collaterals lc
          INNER JOIN collateral_assets ca ON ca.id = lc.collateral_asset_id
          WHERE lc.id = ? AND lc.loan_id = ?
        `,
        [loanCollateralId, loanId],
      );
      if (!linkedCollateral) {
      ...
      await run("DELETE FROM loan_collaterals WHERE id = ? AND loan_id = ?", [loanCollateralId, loanId]);
      ...
      res.status(200).json({
        message: "Collateral unlinked from loan",
  ...
  app.post("/api/loans/:id/collaterals/:loanCollateralId/release", authenticate, authorize(...collateralManageRoles), async (req, res, next) => {
    try {
      ...
      await run("DELETE FROM loan_collaterals WHERE id = ? AND loan_id = ?", [loanCollateralId, loanId]);
      await run(
        "UPDATE collateral_assets SET status = 'released', updated_at = datetime('now') WHERE id = ?",
        [linkedCollateral.collateral_asset_id],
      );
      ...
      const releasedAsset = await get("SELECT * FROM collateral_assets WHERE id = ?", [linkedCollateral.collateral_asset_id]);
      res.status(200).json({
