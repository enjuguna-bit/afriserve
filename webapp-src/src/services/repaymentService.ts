...
function createRepaymentService(deps: RepaymentServiceDeps) {
  ...
  async function allocateScheduledInstallments(
    ...
    const totalScheduledDue = installments.reduce(
      (sum, installment) => sum.plus(toMoneyDecimal(installment.amount_due || 0)),
      toMoneyDecimal(0),
    ).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    const totalScheduledPaidBefore = installments.reduce(
      (sum, installment) => sum.plus(toMoneyDecimal(installment.amount_paid || 0)),
      toMoneyDecimal(0),
    ).toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
    ...
    if (remaining.greaterThan(0.009)) {
      throw new LoanStateConflictError("Repayment allocation exceeded outstanding installment amounts. Please retry.");
    }
  ...
  async function recordRepayment({
    ...
    const repaymentResult = transactionClient
      ...
      : await prisma.$transaction(async (tx) => runRepaymentTx(tx));
