...
async function createBootstrapContext(options: BootstrapContextOptions = {}) {
  ...
  const trustProxyEnabled = String(env.NODE_ENV || "").trim().toLowerCase() === "production" || trustProxyRequested;
  ...
  const hierarchyService = createHierarchyService({
    get,
    all,
    executeTransaction,
  });
  ...
  const loanProductCatalogService = createLoanProductCatalogService({
    get,
    createHttpError,
  });
