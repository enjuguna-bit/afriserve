...
function validateEnvironment(env: NodeJS.ProcessEnv = process.env): ValidationResult {
  ...
  const dbClient = String(env.DB_CLIENT || "sqlite").trim().toLowerCase();
  ...
  const rateLimitRedisUrl = String(env.RATE_LIMIT_REDIS_URL || "").trim();
  ...
  if (jobQueueEnabled && !jobQueueRedisUrl) {
    errors.push("JOB_QUEUE_ENABLED=true requires JOB_QUEUE_REDIS_URL.");
  }
