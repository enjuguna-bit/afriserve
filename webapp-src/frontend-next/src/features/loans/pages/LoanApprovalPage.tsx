...
export function LoanApprovalPage() {
  ...
  if (isLoanOfficer && !canReviewPendingLoans) {
    return (
      <div>
        <h1>Approvals</h1>
        <div style={{ marginTop: '2rem', padding: '1.5rem', background: '#fff0f0', border: '1px solid #ffccd5', borderRadius: '8px', display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <span style={{ fontSize: '1.5rem' }}>🔒</span>
          <div>
            <strong style={{ display: 'block', color: '#d00000', marginBottom: '0.25rem' }}>Access Restricted</strong>
            <p style={{ margin: 0, color: '#9d0208' }}>Your role does not have permission to view or manage loan approvals.</p>
          </div>
        </div>
      </div>
    )
      ...
      {recentlyApprovedLoan ? (
        <section className={styles.handoffBanner} style={{
          ...
          <div className={styles.handoffCopy} style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
            ...
            <div>
              <strong style={{ color: '#065f46', display: 'block', fontSize: '1rem', marginBottom: '0.25rem' }}>
                Loan #{recentlyApprovedLoan.loanId} approved{recentlyApprovedLoan.clientName ? ` for ${recentlyApprovedLoan.clientName}` : ''}.
              </strong>
              <span style={{ color: '#047857', fontSize: '0.875rem' }}>Continue directly to the funding workspace to disburse this loan.</span>
            </div>
          </div>
            ...
            <>
              {selectedLoanIds.size > 0 ? (
                <div style={{
                  ...
                  <span style={{ fontWeight: 600, color: 'var(--color-primary, #2563eb)' }}>{selectedLoanIds.size} loan(s) selected for approval</span>
                  <div style={{ display: 'flex', gap: '0.75rem' }}>
                    <button type="button" onClick={() => setSelectedLoanIds(new Set())} disabled={isBatchApproving} style={{
                      background: 'transparent', color: 'var(--color-muted, #64748b)', border: 'none', cursor: 'pointer'
                    }}>Cancel</button>
                    <button type="button" onClick={batchApprove} disabled={isBatchApproving} style={{
                      background: 'var(--color-primary, #2563eb)', color: '#fff', border: 'none', padding: '0.5rem 1rem', borderRadius: '6px', fontWeight: 600, cursor: 'pointer'
                    }}>
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {pendingLoans.map((loan) => {
                    ...
                    return (
                      <tr key={loan.loan_id} style={isSelected ? { background: 'var(--color-surface-alt, #f8fafc)' } : {}}>
                        <td>
                          <input 
                            type="checkbox"
                            checked={isSelected}
                        ...
                        <td>
                          <div className={styles.metaBlock}>
                            ...
                            <span style={{ color: 'var(--color-amber, #d97706)' }}>In queue: {formatTimeInQueue(loan.submitted_at)}</span>
